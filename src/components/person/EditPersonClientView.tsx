...
export default function EditPersonClientView({
  person,
}: {
  person: Person;
}) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in">
      ...
      <nav className="flex items-center gap-2 text-xs text-[#727973]">
        ...
        <ChevronRight className="w-3.5 h-3.5" />
        <span className="font-semibold text-[#1f1b17]">{t('edit')}</span>
      </nav>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-serif text-3xl font-bold text-[#173124] tracking-tight">
            {t('person_edit_title')} {person.first_name} {person.last_name}
          </h1>
          <p className="text-sm text-[#727973] mt-0.5">
            {t('person_edit_subtitle')}
          </p>
        </div>
      </div>
